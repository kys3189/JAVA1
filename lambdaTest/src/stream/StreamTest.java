...
//		IntStream.range(0, 10).forEach((num1) -> {
...
//		data4.chars().map((c) -> c + 4).forEach(c -> System.out.println((char)c));
		...
		User user1 = new User(1, "김영수", 21, "학생");
...
//		유저의 나이만 출력하기, 참조형x
		users.stream().map(user -> user.getAge()).map((age) -> {return "나이 : " + age;}).forEach(System.out::println);;
...
//		실습
		...
		als.stream().map((str) -> {return "/app" + str;}).forEach(System.out::println);
...
//		결과값의 리턴을 다양한 타입으로 리턴 : collect(Colletor.to~());
